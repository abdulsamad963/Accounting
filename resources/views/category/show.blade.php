...
<a href="{{ route('category.index') }}" class="mr-auto" role="button">رجوع للخلف </a>
.small-table {
...
<form action="{{ route('product.store')}}" method="POST">
    @csrf
    <input type="text" class="form-control" id="num" name="category_id" required value={{$id}} style="display: none;">
    ...
    <table  class="small-table" border="0" cellpadding='0' cellspacing='0' style='text-align: right;' class='table table-hover table-striped'>
        <tr>
            <td><label for="name" class="form-label">اسم المنتج</label>   </td>
            <td><input type="text" class="form-control" id="name" name="name" required> </td>
        </tr>
        <tr>
            <td><label for="major" class="form-label">الوصف</label></td>
            <td><input type="text" class="form-control" id="num" name="description" required> </td>
        </tr>
        <tr>
        <tr>
    <td><label for="major" class="form-label">سعر رأس المال</label></td>
    <td><input type="float" class="form-control" id="num" name="price2" required maxlength="6" oninput="checkPriceLength(this)" > </td>
</tr>
    <td><label for="major" class="form-label">سعر المباع</label></td>
    <td><input type="float" class="form-control" id="num" name="price" required maxlength="6" oninput="checkPriceLength(this)" > </td>
</tr>


        <tr>
            <td><label for="major" class="form-label">الكمية </label></td>
            <td><input type="number" class="form-control" id="num" name="quantity" required> </td>
        </tr>
        <td>
...
<table name="idd" id="idd" border="0" cellpadding='0' cellspacing='0' style='text-align: left;' class='table table-hover table-striped'>
        <thead class="thead-dark">
            <tr >
                ...
                <th scope="col">سعر رأس المال</th>
                <th scope="col">سعر بيع القطع</th>
                <th scope="col">الكمية</th>
                <th scope="col">تاريخ إضافة المنتج</th>
        ...
        <tbody>
            ...
            <tr >
                ...
                <td>
                    <div class="form-group">
                        <input style="width: 100px;" type="text" class="form-control" name="price2" id="price2-{{ $item->id }}" value="{{ $item->price2 }}" disabled>
                    </div>
                </td>
                <td>
                    <div class="form-group">
                ...
                <td>
                    <div class="form-group">
                        <input style="width: 100px;" type="text" class="form-control" name="quantity" id="quantity-{{ $item->id }}" value="{{ $item->quantity }}" disabled>
                    </div>
                </td>
                <td>
                    <div class="form-group">
                        <input style="width: 100px;" type="text" class="form-control" name="description" id="quantity1-{{ $item->id }}" value="{{ $item->created_at }}" disabled>
                    </div>
                </td>
                <td>
                ...
                <td>
        <form action="{{ route('product.destroy',$item->id ) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من رغبتك في حذف هذا المنتج؟')">
    @csrf
    @method('DELETE')
